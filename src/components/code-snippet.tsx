...
function CodeSnippet(props: CodeSnippetProps) {
  return (
    <div className="relative">
      <CopyCode>{props.children}</CopyCode>
      <Code
        {...props}
        theme={theme}
        className="m-0 -mx-[16px] mb-64 border border-[var(--color-card-border)] md:rounded-[8px] [&_pre]:p-[16px] [&_code]:font-mono [&_code>div]:text-[1.125rem] [&_code>div]:p-0"
      />
    </div>
  );
}
